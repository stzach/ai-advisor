...
public class Users : IEndpointGroup
{
    ...
    public static async Task<Ok<UserProfileDto>> Me(UserManager<ApplicationUser> userManager, HttpContext httpContext)
    {
        ...
        return TypedResults.Ok(new UserProfileDto(user?.FirstName, user?.LastName, user?.UserAge ?? 0));
    }
}

public record UserProfileDto(string? FirstName, string? LastName, int UserAge);
